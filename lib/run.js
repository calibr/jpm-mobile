...
function run(manifest, program) {
  ...
  var profileDir;
  var xpiPath = path.join(options.cwd, "ext.xpi");
  var logcatP;
  var id = getID(manifest);
  console.log("jpm-mobile "+ program.command, id);
  ...
  var xpiFileStat;
  try {
    xpiFileStat = fs.statSync(xpiPath);
  }
  catch(ex) {
  }
  return Promise.resolve(xpiFileStat).then(function(stat) {
      if(!stat) {
